...
class TestSubstituteTokens:
    ...
    def test_partial_overlap_substitutes_only_known(self):
        text = "{{CMD_PREFIX}}review {{WAT}}"
        assert _substitute_tokens(text, "opencode") == "osx-review {{WAT}}"
        assert _substitute_tokens(text, "claude") == "osx:review {{WAT}}"
...
class TestDeployLeavesNoLeftoverTokens:
    ...
    def test_deploy_skill_writes_no_tokens(self, target: Path):
        ...
        for md in (target / "skills" / "osx-review-artifacts").rglob("*.md"):
    ...
    def test_deploy_claude_command_substitutes_claude_values(self, tmp_path: Path):
        ...
        for label in labels:
            assert "`/osx:" in label, (
                f"slash-command label must be Claude form `/osx:review` on "
                f"Claude, got: {label!r}"
            )
        assert ".claude/skills/osx-review-artifacts/SKILL.md" in command_text
        assert "osx:review-artifacts" not in command_text
