...
The full list is enumerated in `orchestrator/resources/claude/manifest.toml` under `[resources.commands.*]`.

## Conventions

- The frontmatter `name:` is the source of truth for the slash-command identifier — never rely on the filename alone.
- Phase command files must keep the `osx-phaseN` naming; the orchestrator's `PHASE_COMMANDS` table in `orchestrator/source/orchestrator/engine.py` references these names.

## See Also

- `orchestrator/resources/claude/commands/AGENTS.md` — Command directory conventions
- `orchestrator/resources/opencode/commands/AGENTS.md` — Sibling platform (treat as canonical)
