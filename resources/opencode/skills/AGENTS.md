...
OpenCode source files carry `{{TOKEN}}` placeholders; the deploy step renders them per platform. The substitution table lives in `source/cli.py:PLATFORM_TOKENS` (single source of truth — the bash `sync-mirrors` script is now a pure mirror, no token rewriting):
...
- `{{CMD_PREFIX}}` is **slash-command-only**. Use it inside `/...` form (e.g. `/{{CMD_PREFIX}}modify`). The slash command differs between OpenCode (`osx-`) and Claude (`osx:`).
- **Skill directory paths are hardcoded** `osx-` (works on both platforms — the directory name is identical). Do not write `{{CMD_PREFIX}}modify-artifacts`; write `osx-modify-artifacts`. The previous form produced a broken path on Claude (`osx:modify-artifacts` doesn't exist).

Adding a new token: extend both `PLATFORM_TOKENS` entries in `source/cli.py` and add a regression test in `tests/unit/test_token_substitution.py`. Unknown tokens are left verbatim so future additions surface as literals rather than silently disappearing.
